...
function onMapClick(e) {
    ...
    var tld = e.layer.feature.properties.tld;
    ...
    .setContent(`<a href="/${tld}" onclick="document.getElementById('latlngForm').submit(); return false;" style="text-decoration:none">${city}, ${country} (${id})</a>`)
    .openOn(map);

    var selectform = document.getElementById("latlngForm");

    if (selectform.childNodes.length > 1){
        selectform.childNodes[1].value = requestform.toString();
        selectform.childNodes[2].value = tld;
    } else {
        var inputElem = document.createElement('input');
        inputElem.type = 'hidden';
        inputElem.name = 'latlng';
        inputElem.value = requestform.toString();
        var inputElem1 = document.createElement('input');
        inputElem1.type = 'hidden';
        inputElem1.name = 'tld';
        inputElem1.value = tld;
        selectform.appendChild(inputElem);
        selectform.appendChild(inputElem1);
    }
}
...
L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
    ...
    tileSize: 512,
    zoomOffset: -1,
    accessToken: 'pk.eyJ1IjoiY2Fpb2VzciIsImEiOiJja29jd2FuNTMwbG5nMm51aG43YzYwMjExIn0.7Dfl9d3ED7DhlonIi0cdxA'
}).addTo(map);

var popup = L.popup();

capitals.forEach(capital => {
    L.geoJSON(capital).on('click', onMapClick).addTo(map);
})
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        1,
        3
        )} card-rotation-${getRandomInt(1, 4)}`;
    }
});